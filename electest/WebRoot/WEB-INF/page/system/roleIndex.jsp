  ...
  //     function selectRole(){
	...
	/* 	function displayuser(){
		...
		function displayuser(){
			...
			if($("#dataUser").css("display")=="none"){
				$("#dataUser").css("display","");
				$("#userflag").text("用户权限 关闭");
			}else{
				$("#dataUser").css("display","none");
				$("#userflay").text("用户权限 打开");
			}
		}
		...
		function goSelect(id){
			...
			if(array[0]==array[1]){//此时说明操作的是（父节点）
				//如果选中父
				if($("#"+id)[0].checked){
					//子都选中
			...
			if(array[0]!=array[1]){//此时说明操作的是（子节点）
				//如果选中子节点
				if($("#"+id)[0].checked){
					//父节被选中
					$("input[type='checkbox'][name='selectoper'][id^='"+array[0]+"'][id$='"+array[0]+"']").attr("checked",true);
				}else{
					var checkNum=$("input[type='checkbox'][name='selectoper'][id^='"+array[0]+"']:checked").length;
					if(checkNum==1){
